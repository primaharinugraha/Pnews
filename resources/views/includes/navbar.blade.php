    ...
    <div class="flex gap-10 w-full">
      ...
      <div class="flex items-center justify-between w-full lg:w-auto">
        ...
        <a href="{{route('landingpage')}}">
          <div class="flex items-center gap-2">
            <p class="text-lg lg:text-xl font-bold">P News</p>
          </div>
        </a>
      ...
      <div id="menu"
        ...
        <ul
          ...
          <li>
            <a href="{{ route('landingpage') }}" class="{{ request()->is('/') ? 'text-primary' : 'text-gray-600' }} hover:text-gray-600">

              Beranda
            </a>
          </li>
          @foreach (\App\Models\NewsCategory::all() as $category)
          <li><a href="{{route('newscategory', $category->slug)}}" 
            class="{{request()->is($category->slug) ? 'text-primary': ''}} hover:text-gray-600">{{$category->title}}</a></li>
          @endforeach
